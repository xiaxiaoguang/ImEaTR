...
class EaTR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
                 ...
                 attention_weights_align=False,
                 contrastive_align_loss=False, contrastive_hdim=64,
                 negative_saliency=False,
        ...
        self.saliency_proj1 = MLP(hidden_dim, hidden_dim,hidden_dim,2)
        self.saliency_proj2 = MLP(hidden_dim, hidden_dim,hidden_dim,2)
        ...
        # loss
        self.negative_saliency = negative_saliency
        self.contrastive_align_loss = contrastive_align_loss
        self.attention_weights_align = attention_weights_align
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask):
        ...
        src_txt = self.input_txt_proj(src_txt)  # (bsz, L_txt, d)torch.Size([32, 77, 256])
        ...
        vid_length = src_vid.shape[1]
        txt_length = src_txt.shape[1]

        global_token  = self.global_rep_token.reshape([1, 1, self.hidden_dim]).repeat(src_vid.shape[0], 1, 1)
        src_vid = torch.cat([global_token,src_vid],dim=1) # torch.Size([32, 59, 256])
        
        global_mask  = torch.ones(src_vid_mask.shape[0],1,device = src_vid_mask.device)
        src_vid_mask = torch.cat([global_mask,src_vid_mask],dim=1)
        
        global_position = self.global_rep_pos.reshape(1,1,self.hidden_dim).repeat(src_vid.shape[0],1,1)
        pos_vid = torch.cat([global_position,pos_vid],dim=1)
        ...
        # torch.Size([4, 32, 10, 256]) ,torch.Size([32, 137, 256])torch.Size([3, 32, 10, 2])
        hs, memory, reference, ats, global_token, cross_entropy, dvae_mse  = self.transformer(src, ~mask, pos, src_vid, pos_vid, ~src_vid_mask.bool(), src_txt_global)
        ...
        txt_mem = memory[:, vid_length:]  # (bsz, L_txt, d)
        vid_mem = memory[:, :vid_length]  # (bsz, L_vid, d)
        ...
        if self.attention_weights_align:
            out['attention_weight'] = ats
            out['video_mask'] = src_vid_mask[:,1:]
            
        out['dvae_mse'] = dvae_mse
        out['object_CE'] = cross_entropy
        # breakpoint()
        out["saliency_scores"] = torch.sum(self.saliency_proj1(global_token.unsqueeze(1))*
                                           self.saliency_proj2(vid_mem),dim=-1)/np.sqrt(self.hidden_dim).squeeze(-1)  # (bsz, L_vid)
        # out["saliency_scores"] = self.saliency_proj(vid_mem).squeeze(-1)  # (bsz, L_vid)

        if self.negative_saliency:

            src_txt_neg = torch.cat([src_txt[1:], src_txt[:1]], dim=0)
            src_txt_mask_neg = torch.cat([src_txt_mask[1:], src_txt_mask[:1]], dim=0)
            ...
            src_txt_global = torch.max(src_txt_neg, dim=1)[0]

            _, neg_memory, _, ats2, neg_global_token, _ , _  = self.transformer(src_neg, ~mask_neg, pos_neg, src_vid, pos_vid.clone(), ~src_vid_mask.bool(), src_txt_global)
            neg_vid_mem = neg_memory[:, :vid_length]  # (bsz, L_vid, d)

            out["negative_saliency_scores"] = torch.sum(self.saliency_proj1(neg_global_token.unsqueeze(1))*
                                           self.saliency_proj2(neg_vid_mem),dim=-1)/np.sqrt(self.hidden_dim).squeeze(-1)  # (bsz, L_vid)
            
            # if self.attention_weights_align:
            #     out['attention_weight'] -= ats2
