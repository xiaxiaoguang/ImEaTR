...
class EaTR(nn.Module):
    ...
    # def get_first_non_zero(src):
    #     bsz = src.shape[0]
    #     ret = []
    #     for i in range(bsz):
    #         ret.append(np.nonzero(src[i])[0])
    #     return ret
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask):
        ...
        global_token  = torch.ones(src_vid.shape[0],1,src_vid.shape[2],device=src_vid.device)
        src_vid = torch.cat([global_token,src_vid],dim=1) # torch.Size([32, 59, 256])

        src_vid_mask = torch.cat([torch.zeros(src_vid_mask.shape[0],1,device = src_vid_mask.device),src_vid_mask],dim=1)
        ...
        hs, memory, reference, ats, global_token  = self.transformer(src, ~mask, pos, src_vid, pos_vid, ~src_vid_mask.bool(), src_txt_global)
