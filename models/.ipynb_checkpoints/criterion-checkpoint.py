...
class SetCriterion(nn.Module):
    ...
    def loss_attention_weights_align(self , outputs , targets , indices):
        ...
        lv,ats = outputs['attention_weight']
        bsz = len(targets["span_labels"])
        
        targets = targets["span_labels"]
        tgt_spans = torch.cat([t['spans'][i] for t,(_, i) in zip(targets,indices)], dim=0)
        tgt_spans = torch.round(span_cxw_to_xx(tgt_spans).clamp(0,1-1.0/lv)*lv).to(torch.int32)
        loss = torch.empty((0,),device=tgt_spans.device)

        for i in range(bsz):
            a = tgt_spans[i][0]
            b = tgt_spans[i][1]
            # for j in range(a,b+1):
            #     pos_sum = torch.sum(ats[i][j][lv+1:])
            pos_sum = torch.sum(ats[i][a:b+1])
            allpos = torch.sum(ats[i][:])
            # neg_sum = torch.sum(ats[i][:lv+1,lv+1:])-pos_sum
            # allneg = torch.sum(ats[i][:lv+1])-allpos
            pos_sum = - torch.log(pos_sum / allpos)
            # neg_sum = torch.log(neg_sum / allneg)
            # neg_sum = (torch.sum(ats[i][:a,lv+1:])+torch.sum(ats[i][b+1:,lv+1:]))
            # allneg = (torch.sum(ats[i][:a]))
            # if loss is None:
            #     loss = -torch.log(pos_sum/all_sum).unsqueeze(0)
            # else :
            loss = torch.cat([loss, (pos_sum).unsqueeze(0)],dim=0)
            # if self.rc >=255:
            #     breakpoint()

        losses = {"attention_weights_align":loss.mean()}
        return losses
    ...
    def forward(self, outputs, targets, epoch_i):
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    if loss in ['saliency', 'event_spans' , 'attention_weights_align']:
                        continue
                    kwargs = {}
