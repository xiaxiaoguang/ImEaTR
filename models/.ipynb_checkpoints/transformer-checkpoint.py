...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, pos_embed, src_vid, pos_embed_vid, src_vid_mask, src_txt_global):
        ...
        hs, references = self.decoder(memory_local, memory_key_padding_mask=mask, 
                                      pos=pos_embed, 
                                      src_vid=src_vid,
        ...
        memory_local = memory_local.transpose(0, 1)  # (bsz, L, d)

        return hs, memory_local, references , ats , global_token
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src,
        ...
        # if src_mask is None :
        #     src_mask = ~torch.matmul((~src_key_padding_mask).float().unsqueeze(2),
        #                             (~src_key_padding_mask).float().unsqueeze(1)).bool().repeat(self.nhead,1,1)
