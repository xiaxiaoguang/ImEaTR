...
def pad_sequences_1d(sequences, dtype=torch.long, device=torch.device("cpu"), val=0,fixed_length=None):
    ...
    if isinstance(sequences[0], torch.Tensor):
        ...
        padded_seqs = torch.full((len(sequences), max_length) + extra_dims, val , dtype=dtype, device=device)
        mask = torch.zeros((len(sequences), max_length), dtype=torch.float32, device=device)
    else:  # np
        assert "numpy" in str(dtype), "dtype and input type does not match"
        padded_seqs = np.full((len(sequences), max_length) + extra_dims,val, dtype=dtype)
